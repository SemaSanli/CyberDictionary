...
sozluk = {
    "elma": "Bir tür meyve",
    "bilgisayar": "Veri işleme cihazı",
    "kitap": "Okumak için yazılı materyal"
}
...
# Başlık ve açıklama
tk.Label(pencere, text="CyberDict", font=('Arial', 24, 'bold'), bg="#2c3e50", fg="white").grid(row=0, column=0, columnspan=3, pady=(20, 10))
tk.Label(pencere, text="Bu program ile siber güvenlikte kullanılan terimlerin anlamlarını arayabilir, yeni kelimeler ekleyebilir ve silerek kendi sözlüğünüzü oluşturabilirsiniz.", font=('Arial', 14), bg="#2c3e50", fg="white").grid(row=1, column=0, columnspan=3, pady=(0, 20))
...
# Kelime girişi
tk.Label(pencere, text="Kelime:", font=('Arial', 14), bg="#2c3e50", fg="white").grid(row=2, column=0, padx=10, pady=10, sticky='e')
...
tk.Label(pencere, textvariable=sonuc_var, font=('Arial', 14), bg="#2c3e50", fg="white").grid(row=3, column=0, columnspan=2, padx=10, pady=10)
...
tk.Label(pencere, text="Yeni Kelime:", font=('Arial', 14), bg="#2c3e50", fg="white").grid(row=5, column=0, padx=10, pady=10, sticky='e')
yeni_kelime_giris = tk.Entry(pencere, font=('Arial', 14))
yeni_kelime_giris.grid(row=5, column=1, padx=10, pady=10, sticky='w')

tk.Label(pencere, text="Anlam:", font=('Arial', 14), bg="#2c3e50", fg="white").grid(row=6, column=0, padx=10, pady=10, sticky='e')
yeni_anlam_giris = tk.Entry(pencere, font=('Arial', 14))
yeni_anlam_giris.grid(row=6, column=1, padx=10, pady=10, sticky='w')


# Pencereyi çalıştır
guncelle_liste()  # Başlangıçta listeyi güncelle
pencere.mainloop()


def kelime_arama(event=None):
    kelime = kelime_giris.get().strip().lower()
    if kelime in sozluk:
        sonuc_var.set(f"{kelime}: {sozluk[kelime]}")
    else:
